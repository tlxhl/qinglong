...
[task_local]
#签到领现金
2 5,9,16,20 * * * jd_cash.js, tag=签到领现金, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true

================Loon==============
[Script]
cron "2 5,9,16,20 * * *" script-path=jd_cash.js,tag=签到领现金

===============Surge=================
签到领现金 = type=cron,cronexp="2 5,9,16,20 * * *",wake-system=1,timeout=3600,script-path=jd_cash.js

============小火箭=========
签到领现金 = type=cron,script-path=jd_cash.js, cronexpr="2 5,9,16,20 * * *", timeout=3600, enable=true
 */
const $ = new Env('签到领现金');
...
const inviteCodes = [
  `eU9Ya7-6Z60j9z3RznFFhA@eU9YDpPsHqJOiimGoTFx`,
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `http://code.chiang.fun/api/v1/jd/jdcash/read/${randomCount}/`, 'timeout': 3000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
