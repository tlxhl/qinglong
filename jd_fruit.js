...
[task_local]
#jd免费水果
5 3,11,18 * * * jd_fruit.js, tag=东东农场, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdnc.png, enabled=true
=========================Loon=============================
[Script]
...
let shareCodes = [ // 这个列表填入你要助力的好友的shareCode
  ...
  'd058b014e2b5445dbf5fbbf661c84de1',
  //账号二的好友shareCode,不同好友的shareCode中间用@符号隔开
  '493a11a8af284d3d99a2c11e6ae09922',
]
let message = '', subTitle = '', option = {}, isFruitFinished = false;
const retainWater = 100;//保留水滴大于多少g,默认100g;
let jdNotify = false;//是否关闭通知，false打开通知推送，true关闭通知推送
let jdFruitBeanCard = false;//农场使用水滴换豆卡(如果出现限时活动时100g水换20豆,此时比浇水划算,推荐换豆),true表示换豆(不浇水),false表示不换豆(继续浇水),脚本默认是浇水
let randomCount = $.isNode() ? 2 : 5;
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `https://lq.d20q2.cn/jd/farm.json`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))
